...
create or replace function blqs_de_mzas(componentes text, adyacencias text)
returns table (set_of_mza integer[]) as $$
begin
return query execute
format('    
    ...
    or array_length(set_of_mza,1) = 2
        and (set_of_mza[1], set_of_mza[2]) in (select * from %2$s)
    or array_length(set_of_mza,1) = 3
        and (set_of_mza[1], set_of_mza[2]) in (select * from %2$s)
        and (set_of_mza[2], set_of_mza[3]) in (select * from %2$s)
    or array_length(set_of_mza,1) = 3
        and (set_of_mza[1], set_of_mza[2]) in (select * from %2$s)
        and (set_of_mza[1], set_of_mza[3]) in (select * from %2$s)
    ', $1, $2);
end
$$ language plpgsql;
