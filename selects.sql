...
with deseado as (
    ...
    buscando_diferencias as (
    select comunas, frac_comun, radio_comu::integer, mza_comuna::integer,
           count(*) as vivs,
           ceil(count(*)/deseado) as ceil_seg_mza,
           floor(count(*)/deseado) as floor_seg_mza
    ...
    select *, 
        case 
            when floor_seg_mza = 0 then vivs
            else vivs < least(vivs/ceil_seg_mza - deseado, vivs/floor_seg_mza - deseado)
        end as mas_cerca
    from buscando_diferencias, deseado
    )
select *
...
with deseado as (
    ...
    select vivs, floor(vivs/deseado) as min, ceil(vivs/deseado) as max
    from casos, deseado
    )
select * from posibles_segs_mza
;
    ...
    parejo as (
    select round(cant/deseado)*deseado as redondo
    from sumados, deseado
    )
select frac_comun, radio_comu, mza_comuna, clado, hn, hp, ceil(rank/maximo_manzana) as segmento_manzana
from deseado_manzana
join separados
using(frac_comun, radio_comu, mza_comuna)
left join sumados
