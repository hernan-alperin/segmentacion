...
try {
    ...
    $commands = array(
       ...
       "{$ogr_path}/shp2pgsql -W latin1 -s 22183 -d $dir/{$fileName}a shapes.\"{$fileName}a\" | python shp2table.py",
       "{$ogr_path}/shp2pgsql -W latin1 -s 22183 -d $dir/{$fileName}p shapes.\"{$fileName}p\" | python shp2table.py",
    ...
    echo date("Y-m-d H:i:s");
    echo "---- Generando conteos y adyacencias\n";

    $command = "python generate_structure.py {$fileName}a";
    TryExec($command, "Error", $out);
...
EOT;
    ...
    $command = "{$ogr_path}/pgsql2shp -f $dir/{$fileName}a -u segmentador -P rodatnemges censo2020 shapes.\"{$fileName}a\"";
    TryExec($command, "Error", $out);
