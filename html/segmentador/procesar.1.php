...
try {
    ...
    $commands = array(
       ...
       "{$ogr_path}/shp2pgsql -W latin1 -s 22183 -d $dir/{$fileName}a shapes.\"{$fileName}_a\" | (export PGPASSWORD=halpe; psql halpe -U daemon)",
       "{$ogr_path}/shp2pgsql -W latin1 -s 22183 -d $dir/{$fileName}p shapes.\"{$fileName}_p\" | (export PGPASSWORD=halpe; psql halpe -U daemon)",
    ...
    $sql = "select distinct prov, depto from shapes.\"{$fileName}_p\";";
...
EOT;
    ...
    $queries = array(
        "alter table shapes.\"{$fileName}_p\" add column segmento integer;",
        "alter table shapes.\"{$fileName}_a\" add column segi integer;",
        "alter table shapes.\"{$fileName}_a\" add column segd integer;"
        );
    ...
    $sql = <<<EOT
update shapes.{$fileName}_a
set segi = segmento
from shapes.{$fileName}_p
where prov::integer = $prov and depto::integer = $depto
...
EOT;
    ...
    $command = "{$ogr_path}/pgsql2shp -f $dir/{$fileName}a halpe shapes.\"{$fileName}_a\"";
    TryExec($command, "Error", $out);
